...
export function calculateProbabilities(
    ...
    const blockedCellsSet = new Set(
        blockedCells.map(({ x, y }) => `${x},${y}`)
    );
    ...
    for (const obj of objects) {
        ...
        for (const placement of validPlacements) {
            for (const cell of placement.cells) {
                ...
                cellCoverage.set(
                    key,
                    (cellCoverage.get(key) || 0) + placement.weight
                );
            }
        }

        // Calculate probability for each cell
        const totalWeight = validPlacements.reduce(
            (sum, p) => sum + p.weight,
            0
        );
        ...
        for (const [cellKey, coverage] of cellCoverage.entries()) {
            ...
            probabilities[y][x] =
                1 - (1 - probabilities[y][x]) * (1 - atLeastOneProb);
        }
    }
